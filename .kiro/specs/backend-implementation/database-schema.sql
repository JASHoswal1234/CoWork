...
--   AND ST_DWithin(w.location, ST_SetSRID(ST_MakePoint(77.2090, 28.6139), 4326)::geography, w.service_radius * 1000)
-- ORDER BY distance_km
-- LIMIT 10;

-- Worker earnings by month
-- COMMENT ON QUERY IS:
-- SELECT DATE_TRUNC('month', p.paid_at) AS month, SUM(p.worker_earnings) AS total_earnings
-- FROM payments p
-- WHERE p.worker_id = 'some-worker-uuid'
